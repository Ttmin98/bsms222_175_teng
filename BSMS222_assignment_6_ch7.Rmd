---
title: "BSMS222_assignment_6_ch7"
output:
  html_document:
    df_print: paged
---
...
<img src='https://rafalab.github.io/dsbook/book_files/figure-html/ggplot-example-plot-1.png' width='auto'>
...
<img src='https://rafalab.github.io/dsbook/R/img/ggplot2-cheatsheetb.png' width='auto'>
...
```{r}
p + geom_point(aes(population/10^6, total)) +
  geom_text(aes(population/10^6, total, label = region))
```
...
p + geom_point(aes(population/10^6, total), size = 3) +
  geom_text(aes(population/10^6, total, label = abb))
# size는 aes function 에 속하지 않음 
# point에 들어있는 size는 굵기 text 안에 있는건 크기
```
...
# nudge_x: 글씨를 옆으로 이동(>1일 때 오른편으로;<1일 때 왼편)
```
...
Here we want to add a line that represents the average murder rate for the entire country. Once we determine the per million rate to be $r$, this line is defined by the formula:  
$y=rx$, with $y$ and $x$ our axes: total murders and population in millions, respectively. In the log-scale this line turns into: $log(y) = log(r) + log(x)$. So in our plot it’s a line with slope 1 and intercept $log(r)$. To compute this value, we use our **dplyr** skills:
```{r}
r <- murders %>% 
...
We will learn more about `qplot` in Section [8.16](https://rafalab.github.io/dsbook/distributions.html#other-geometries)
...
```{r}
# label: add text to the plot with a rectangle behind the text
s <- murders %>% ggplot() + geom_point(aes(population, total))
s+geom_label(aes(population, total, label = abb))
```
```{r}
